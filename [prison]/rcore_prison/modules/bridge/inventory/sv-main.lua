...
Bridge.AddItem = function(playerId, itemName, itemCount, metadata, slot)
    ...
    if state then
        ...
        elseif HasResource('ND_Core') and not hasOxInventory and itemName == 'money' then
            local playerND = NDCore.getPlayer(source)

            if not playerND then
                return
            end

            return playerND.addMoney("cash", itemCount, "Prison - Add Item")
        ...
        elseif HasResource(Prison.InventoryScripts.QS_INVENTORY) and Prison.Framework == FRAMEWORK_MAP.QBCORE then
            ...
            if version and version >= '2.0.6' then
                ...
                if itemName == 'money' and not hasOxInventory and itemList and not (itemList['money'] or itemList['cash']) then
                    return Player.Functions.AddMoney('cash', itemCount)
                end
        ...
        elseif HasResource(Prison.InventoryScripts.CODEM_INVENTORY) then
            if Prison.Framework == FRAMEWORK_MAP.QBCORE then
                ...
                return exports['codem-inventory']:AddItem(playerId, itemName:lower(), itemCount, slot, metadata or {})
...
Bridge.RemoveItem = function(playerId, itemName, itemCount, metadata, slot)
    ...
    if state then
        ...
        elseif HasResource('ND_Core') and not hasOxInventory and itemName == 'money' then
            ...
            return playerND.deductMoney("cash", itemCount, "Prison - Remove money")
        elseif HasResource(Prison.InventoryScripts.MF_INVENTORY) and Prison.Framework == FRAMEWORK_MAP.ESX then
        ...
        elseif HasResource(Prison.InventoryScripts.ESX_INVENTORY) and not hasOxInventory then
            if Player then
                if itemName == 'money' then
                    Player.removeAccountMoney('cash', tonumber(itemCount))
                else
                    Player.removeInventoryItem(itemName, itemCount)
                end
            end
...
Bridge.GetItem = function(playerId, itemName)
    ...
    if state then
        ...
        if Prison.Framework == FRAMEWORK_MAP.ESX then
            ...
            if itemName == 'money' then
                return Player.getAccount('cash') and Player.getAccount('cash').money or 0
            end
        ...
        elseif Prison.Framework == FRAMEWORK_MAP.QBCORE then
            if itemName == 'money' then
                local data = Player.Functions.GetMoney and Player.Functions.GetMoney('cash') or 0
        ...
        elseif IsScriptLoaded('ND_Core') and not hasOxInventory then
            local playerND = NDCore.getPlayer(playerId)

            if itemName == 'money' and not hasOxInventory then
                local data = playerND and playerND.cash or 0
...
Bridge.ReturnPrisonerItemsFromStash = function(src)
    ...
    if decodedStashItems and next(decodedStashItems) then
        ...
        Bridge.ReturnVirtualMoney(src, player.charId)
