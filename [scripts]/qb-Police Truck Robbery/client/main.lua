...
Citizen.CreateThread(function()
    exports['qb-target']:SpawnPed({
        model = Config.HackerModel,
        coords = Config.HackerLocation, 
        minusOne = true, --may have to change this if your ped is in the ground
        freeze = true, 
        invincible = true, 
        blockevents = true,
        scenario = Config.HackerScenario,
        target = { 
            options = {
                {type = "server",event = "AttackTransport:akceptujto",icon = "fa-solid fa-hat",label = "Start Heist",},
            },
          distance = 2.5,
        },
    })
...
RegisterNetEvent('AttackTransport:InfoForLspd', function(x, y, z)
	if PlayerJob ~= nil and PlayerJob.name == 'police' then
		if PoliceBlip == 0 then
			...
			AddTextComponentString('Assault on shipment of confiscated items')
...
function MissionNotification()
	...
	TriggerServerEvent('qb-phone:server:sendNewMail', {
		sender = "Hacker",
		subject = "New Task",
		message = "A police bus full of confiscated items is leaving shortly, if you are buzz and precise you will win a lot, I have sent you a location from where it is leaving! Good luck!.",
	})
end
---
--
	...
	CreateThread(function()
		...
		while not spawned do
			...
			if distance < 200 then
				spawned = true
				RequestModel(`riot`)
				while not HasModelLoaded(`riot`) do
					Wait(0)
				end
				ClearAreaOfVehicles(VehicleCoords.x, VehicleCoords.y, VehicleCoords.z, 15.0, false, false, false, false, false)
				transport = CreateVehicle(`riot`, VehicleCoords.x, VehicleCoords.y, VehicleCoords.z, 52.0, true, true)
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString('Van with Items')
				...
				pilot = CreatePed(26, "s_m_y_swat_01", VehicleCoords.x, VehicleCoords.y, VehicleCoords.z, 268.9422, true, false)
				navigator = CreatePed(26, "s_m_y_swat_01", VehicleCoords.x, VehicleCoords.y, VehicleCoords.z, 268.9422, true, false)
				navigator2 = CreatePed(26, "s_m_y_swat_01", VehicleCoords.x, VehicleCoords.y, VehicleCoords.z, 268.9422, true, false)
			...
			if dist <= 55.0  then
				...
				if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
					if BlowBackdoor == 0 then
						hintToDisplay('Press [G] to blow up the back door and peel the truck')
	...
	if IsVehicleStopped(transport) then
		if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
			if not IsEntityInWater(PlayerPedId()) then
				...
				BlownUp = 1
				lootable = 1
				QBCore.Functions.Notify('You can start collecting the items.', "success")
				RemoveBlip(TruckBlip)
			else
...
CreateThread(function()
    while true do
		...
		if lootable == 1 then
			...
			if dist <= 4.5 then
				sleep = 0
				hintToDisplay('Press [E] to get the items')
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	QBCore.Functions.Notify('You put the thing in the bag', "success")
