...
RegisterNetEvent("919-admin:server:ReportReply", function(data)
    if data.name ~= nil then
        for _, playerId in ipairs(GetPlayers()) do
            ...
            local charInfo = Compat.GetCharacterData(playerId)
            if charInfo.CharacterIdentifier == data.name then
                TriggerClientEvent("chat:addMessage", playerId, {
                    template = "<div class=\"chat-message server\">[STAFF] New Report Reply: <strong>{0}</strong></div>",
                    args = {data.message}
                })
            end
        end
...
Compat.CreateCallback("919-Admin:GetPlayerName", function(source, cb, id)
    local result = Compat.GetCharacterData(id)
...
RegisterNetEvent("919-admin:server:RequestJobPageInfo", function()
    ...
    if AdminPanel.HasPermission(src, "jobpage") then
        TriggerLatentClientEvent("919-admin:client:ReceiveJobPageInfo", src, 100000, Compat.GetMasterEmployeeList())
    end
end)
...
RegisterNetEvent("919-admin:server:RequestGangPageInfo", function()
    ...
    if AdminPanel.HasPermission(src, "gangpage") then
        TriggerLatentClientEvent("919-admin:client:ReceiveGangPageInfo", src, 100000, Compat.GetMasterGangList())
    end
end)
                ...
                elseif v.accounts then
                    ...
                    if money then
                        for type,amount in pairs(money) do
                            if type == "money" then
                                ServerMetrics.TotalCash = ServerMetrics.TotalCash + amount
                            end
                        end
                    end
                end
            ...
            for k,v in pairs(results) do
                ...
                elseif v.accounts then
                    ...
                    if money then
                        for type,amount in pairs(money) do
                            if type == "bank" then
                                ServerMetrics.TotalBank = ServerMetrics.TotalBank + amount
                            end
                        end
                    end
                end
