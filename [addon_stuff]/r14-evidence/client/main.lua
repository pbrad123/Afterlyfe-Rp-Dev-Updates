...
if Config.Framework.QB then
    ...
    if not QBCore.Shared then print('Cannot access QBCore.Shared object, something has gone terribly wrong!!!! Please make sure your qb-core resources starts without errors.') end

    if not QBCore.Shared.Weapons then print('Cannot read your qb-core/shared/weapons.lua, something has gone terribly wrong!!!! If you added calibers you may have done something wrong, please make sure your qb-core resources starts without errors.') end
                        ...
                        if CurrentEvidence.evtype ~= 'fingerprint' or not Config.Fingerprints.RequireKit then -- check for fingeprint kit requirement
                            ...
                            CreateThread(function()
                                ...
                                if not hasKit or not (hasTape or hasMikrosil) then
                                    if not hasKit then Config.Functions.Notify('You do not have anything to develop this fingerprint.', 'error') end
                                    if not hasTape and not hasMikrosil then Config.Functions.Notify('You do not have anything to lift this fingerprint.', 'error') end                                   
                                    return 
                                end

                                Config.Functions.Progressbar("liftprint", "Developing/lifting fingerprint...", 5000, false, true, {
                                    ...
                                    if CurrentEvidence and CurrentEvidence.evidenceId == startEvidenceId and CurEvLoopRunning then
                                        ...
                                        Config.Functions.Notify("You seem to have moved too far away!", 'error')
                                    end
                                end, function() -- Cancel
                                    ClearPedTasks(ped)
                                end)                              
    ...
    if not casingtimeout and shooter == ped then
        ...
        if not Config.NoAlertWeapon[weapon] then
            if Config.ExplosiveDevice[weapon] then
				if Config.Alerts.Active then TriggerAlert('ExplosiveDevice') end
            elseif weapon == `weapon_firework` then
                if Config.Alerts.Active then TriggerAlert('IllegalFireworks') end
            else
                if IsPedInAnyVehicle(ped) then
                    if Config.Alerts.Active then TriggerAlert('DriveBy') end
                elseif not IsPedInAnyVehicle(ped) then
                    if Config.Alerts.Active then TriggerAlert('ShotsFired') end
                end
            end
        end
    end
...
RegisterNetEvent('evidence:client:accesstool', function(data)
    ...
    Config.Functions.Progressbar("slimjim", "Accessing vehicle...", 2500, false, true, {
        disableMovement = true,
...
RegisterNetEvent('police:client:CheckStatus', function() --- this is so qb-radial menu works the same way as the target function   
    Config.Functions.Progressbar("investigating", "Investigating person...", 5000, false, true, {
...
RegisterNetEvent('evidence:client:breathalyze', function(data)
    ...
    Config.Functions.Progressbar("breathalyzing", "Using Breathalyzer...", 5000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:drugtest', function(data)
    ...
    Config.Functions.Progressbar("drugtesting", "Administring Multipanel Oral Drug Test...", 5000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:fingerprint', function(data)
    ...
    Config.Functions.Progressbar("fingerprinting", "Taking Fingerprint...", 5000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:inspectvehicle', function(data)
    ...
    Config.Functions.Progressbar("inspecting", "Inspecting vehicle...", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:GSRtest', function(data)
    ...
    Config.Functions.Progressbar("gsrtest", "Preforming GSR test...", 4000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:DNAswab', function(data)
    ...
    Config.Functions.Progressbar("dnaswab", "Preforming DNA Swab...", 5000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:frisk', function(data)
    ...
    Config.Functions.Progressbar("frisk", "Preforming Terry Frisk...", 5000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:ClearEvidenceInArea', function()
    Config.Functions.Progressbar('clear_casings', 'Cleaning up crime scene...', 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
...
RegisterNetEvent('evidence:client:bagevidence', function(inventory)
    ...
    for k, v in pairs(inventory) do           
        if v.type == 'weapon' or (Config.Inventory.Ox and weaponList[v.name] and weaponList[v.name].weapon) then
            ...
            local ammo = v[inventoryMetadata].ammo and (', Ammo: %s'):format(v[inventoryMetadata].ammo) or ''
    ...
    OpenContext(bagMenu, 'Bag Evidence In Inventory')
end)
...
RegisterNetEvent('evidence:client:cleanweapon', function(inventory)
    ...
    if curwephash == GetSelectedPedWeapon(ped) then
        ...
        if Config.EvidenceCleanup.AlwaysAvailable or foundRag then
            weaponMenu[#weaponMenu+1] = {
                header = 'Wipe Down Weapon',
                text = 'Clean this weapon of any potential fingerprints.',
                title = 'Wipe Down Weapon',
                description = 'Clean this weapon of any potential fingerprints.',
                params = {
        ...
        for k, v in pairs(foundCleansers) do
            weaponMenu[#weaponMenu+1] = {
                header = 'Clean Weapon',
                text = ('Use %s to clean this weapon'):format(string.lower(k)),
                title = 'Clean Weapon',
                description = ('Use %s to clean this weapon'):format(string.lower(k)),
                params = {
                    event = 'evidence:client:wipeweapon',
                    args = {
                        slot = curslot,
    ...
    elseif IsPedInAnyVehicle(ped) then
        ...
        local weaponMenu = {
            {
                header = 'Vehicle Cleaning Actions',
                isMenuHeader = true,
            }
        }
        ...
        if Config.EvidenceCleanup.AlwaysAvailable or foundRag then
            weaponMenu[#weaponMenu+1] = {
                header = 'Wipe Down Seat',
                text = 'Clean this seat of any potential fingerprints.',
                title = 'Wipe Down Seat',
                description = 'Clean this seat of any potential fingerprints.',
                params = {
        ...
        for k, v in pairs(foundCleansers) do
            weaponMenu[#weaponMenu+1] = {
                header = 'Clean Seat',
                text = ('Use %s to clean this seat'):format(string.lower(k)),
                title = 'Clean Seat',
                description = ('Use %s to clean this seat'):format(string.lower(k)),
                params = {
                    event = 'evidence:client:wipearea',
                    args = {
                        clean = k,
        ...
        OpenContext(weaponMenu, 'Vehicle Cleaning Actions')
    else
        local weaponMenu = {
            {
                header = 'Cleaning Actions',
                isMenuHeader = true,
            }
        }
        ...
        if Config.EvidenceCleanup.AlwaysAvailable or foundRag then
            weaponMenu[#weaponMenu+1] = {
                header = 'Wipe Down Area',
                text = 'Clean this area of any potential fingerprints.',
                title = 'Wipe Down Area',
                description = 'Clean this area of any potential fingerprints.',
                params = {
                    event = 'evidence:client:wipearea',
                },
                event = 'evidence:client:wipearea',
        ...
        for k, v in pairs(foundCleansers) do
            weaponMenu[#weaponMenu+1] = {
                header = 'Clean Area',
                text = ('Use %s to clean the area'):format(string.lower(k)),
                title = 'Clean Area',
                description = ('Use %s to clean the area'):format(string.lower(k)),
                params = {
                    event = 'evidence:client:wipearea',
                    args = {
                        clean = k,
        ...
        OpenContext(weaponMenu, 'Cleaning Actions')
    end
end)
...
RegisterNetEvent('evidence:client:wipearea', function(data)
    ...
    Config.Functions.Progressbar("cleanscene", data?.clean and 'Cleaning area...' or 'Wiping down area...', wipeareaTime, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:wipeweapon', function(data)
    Config.Functions.Progressbar("wipeweapon", data.clean and 'Cleaning weapon...' or 'Wiping down weapon...', 4000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:unbagev', function(slot)
    ...
    Config.Functions.Progressbar("unbagev", "Unbagging evidence...", ActiveLEO and 10000 or 60000, false, true, { -- finish
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
function ConfirmNoExploit(slot)
    if not ActiveLEO then
        ...
        bagMenu[#bagMenu+1] = {
            ...
            title = 'Proceed. I am not exploiting the evidence bag mechanic to reduce my carry weight or get a weapon somewhere I should not have it.',
            params = {
        ...
        bagMenu[#bagMenu+1] = {
            ...
            title = 'Cancel. I have rethought this action, I should not have been able to carry this weapon to where I am.',
            params = {
                event = 'qb-menu:client:closeMenu',
            },
            event = 'qb-menu:client:closeMenu',
        ...
        OpenContext(bagMenu, 'Please Confirm You Are Not Exploiting')
    else
...
RegisterNetEvent('evidence:client:BagMenu', function(data)
    ...
    if data.item[inventoryMetadata].item and (latentprints or latentblood) then
        bagMenu[#bagMenu+1] = {
            header = 'Latent Evidence',
            title = '**Latent Evidence**',
            isMenuHeader = true,
        }
        ...
        if data.item[inventoryMetadata].item[inventoryMetadata].latentprints then
            for k, v in pairs(data.item[inventoryMetadata].item[inventoryMetadata].latentprints) do
                bagMenu[#bagMenu+1] = {
                    header = ('Latent Print %s'):format(v.evidenceId),
                    title = ('Latent Print %s'):format(v.evidenceId),
                    params = {
        ...
        if data.item[inventoryMetadata].item[inventoryMetadata].latentblood then
            for k, v in pairs(data.item[inventoryMetadata].item[inventoryMetadata].latentblood) do
                bagMenu[#bagMenu+1] = {
                    header = ('Latent Blood %s'):format(v.evidenceId),
                    title = ('Latent Blood %s'):format(v.evidenceId),
                    params = {
                        event = 'evidence:client:collectlatent',
                        args = {
                            slot = data.item.slot,
    ...
    bagMenu[#bagMenu+1] = {
        header = 'Evidence Bag Actions',
        title = '**Evidence Bag Actions**',
        isMenuHeader = true,
    }
    ...
    if retrieveItem then
        bagMenu[#bagMenu+1] = {
            ...
            title = ('Unbag %s'):format(retrieveItem),
    ...
    OpenContext(bagMenu, 'Evidence Bag Menu')
end)
...
RegisterNetEvent('evidence:client:collectlatent', function(data)
    ...
    if not hasBag then Config.Functions.Notify('You do not have an evidence bag to collect this with!', 'error') return end
    ...
    if Config.Fingerprints.RequireKit and data.evtype == 'fingerprint' and (not hasKit or not hasMikrosil) then
        ...
        if not hasMikrosil then 
            if Config.Functions.SearchInventoryClient('fingerprinttape') then
                Config.Functions.Notify('This surface requires a different method of transfer in order to lift a print.', 'error')
            else
                Config.Functions.Notify('You do not have anything to lift this fingerprint.', 'error')
            end
        end     
        return 
    end    

    Config.Functions.Progressbar("collectlatent", "Collecting latent evidence..", 3000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('evidence:client:EditNotes', function(data)
    ...
    local bagMenu = {
        {
            header = ('%s Notes'):format(data.item[inventoryMetadata].tracking),
            isMenuHeader = true,
        }
    }
    ...
    bagMenu[#bagMenu+1] = {
        header = ('Created: %s'):format(data.item[inventoryMetadata].notes[data.note].created.name),
        txt = data.item[inventoryMetadata].notes[data.note].created.timestamp,
        title = ('Created: %s'):format(data.item[inventoryMetadata].notes[data.note].created.name),
    ...
    if data.item[inventoryMetadata].notes[data.note].edited then
        bagMenu[#bagMenu+1] = {
            header = ('Last Edited: %s'):format(data.item[inventoryMetadata].notes[data.note].edited.name),
            txt = data.item[inventoryMetadata].notes[data.note].edited.timestamp,
            title = ('Last Edited: %s'):format(data.item[inventoryMetadata].notes[data.note].edited.name),
    ...
    OpenContext(bagMenu, ('%s Notes'):format(data.item[inventoryMetadata].tracking))
end)
...
RegisterNetEvent('evidence:client:NoteInput', function(data)
    ...
    local noteinput = OpenInput({
        header = ('Create Note for %s'):format(data.item[inventoryMetadata].tracking),
        submitText = "Submit",
        inputs = {
            {
                text = 'Text',
    ...
    }, ('Create Note for %s'):format(data.item[inventoryMetadata].tracking))

    local edited = nil

    if noteinput ~= nil then
        if not data.note then
            data.note = #data.item[inventoryMetadata].notes + 1 

            data.item[inventoryMetadata].notes[data.note] = {
...
local function CarEvidenceMenu(evtable, plate)
    ...
    Config.Functions.TriggerCallback('police:GetIgnitionTampering', function(result)
        if result then
            evMenu[#evMenu+1] = {
                ...
                title = 'Vehicle ignition may have been tampered with.',
                disabled = true,
            }
        end
        tamper1 = true
    ...
    Config.Functions.TriggerCallback('police:GetExteriorTampering', function(result)
        if result then
            evMenu[#evMenu+1] = {
                ...
                title = 'Vehicle may have been forcibly entered.',
                disabled = true,
            }
        end
        tamper2 = true
        ...
        if not v.ignore then
            ...
            if v.evtype == 'carcasing' then
                evMenu[#evMenu+1] = {
                    ...
                    title = ('Casing %s, Tracking ID: %s'):format(string.upper(v.serial), k),
                    disabled = noevbag,
                    text = noevbag and '[MISSING EVIDENCE BAG]' or '',
                    description = noevbag and '[MISSING EVIDENCE BAG]' or '',
                    params = {
            ...
            elseif v.evtype == "carfingerprint" then
                evMenu[#evMenu+1] = {
                    ...
                    text = ('Found on %s %s'):format(v.location, nokit and '[MISSING FINGERPRINT KIT]' or ''),
                    title = ('Fingerprint %s, Tracking ID: %s'):format(string.upper(v.fingerprint), k),
                    description = ('Found on %s %s'):format(v.location, nokit and '[MISSING FINGERPRINT KIT]' or ''),
            ...
            elseif v.evtype == 'carblood' then
                evMenu[#evMenu+1] = {
                    ...
                    text = ('Found near Seat %s %s'):format(v.seat, noevbag and '[MISSING EVIDENCE BAG]' or ''),
                    title = ('Blood %s, Tracking ID: %s'):format(string.upper(v.dna), k),
                    description = ('Found near Seat %s %s'):format(v.seat, noevbag and '[MISSING EVIDENCE BAG]' or ''),
                    disabled = noevbag,
                    params = {
                        event = 'evidence:client:collectcarevidence',
                        args = {
    ...
    OpenContext(evMenu, ('Collect Evidence from %s'):format(plate))
end
...
RegisterNetEvent('evidence:client:checkcarevidence', function(data)  
    ...
    Config.Functions.Progressbar("checkingcar", "Investigating vehicle..", 10000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
                ...
                if usingflashlight then
                    ...
                    if next(AreaEvidence) then
                        ...
                        for k, v in pairs(AreaEvidence) do
                            ...
                            if ActiveLEO and (dist < pudist and v.pickup) and v.bucket == curbucket then
